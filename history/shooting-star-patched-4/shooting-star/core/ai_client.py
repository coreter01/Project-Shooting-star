...
SYSTEM_PROMPT = """Bạn là trợ lý dòng lệnh cho Linux, tên "Astra".
...
def ask(brain: str, api_keys: dict, ollama_url: str, history: list, ollama_model: str = "", gemini_model: str = "") -> str:
    if brain == "gemini":
        return _ask_gemini(api_keys.get("gemini", ""), history, gemini_model)
    if brain == "openai":
...
def _ask_gemini(key: str, history: list, model: str = "") -> str:
    if not key:
        return json.dumps({"reply": "Chưa có Gemini API key. Vào Cài đặt để nhập.", "action": None})
    model = model or "gemini-3.6-flash"  # gemini-2.0-flash đã bị khai tử 1/6/2026
    try:
        ...
        if "error" in data:
            return json.dumps({
                "reply": f"Lỗi Gemini: {data['error'].get('message')}. Nếu là lỗi 'model not found', "
                         f"model '{model}' có thể đã bị Google khai tử — kiểm tra ai.google.dev/gemini-api/docs/deprecations "
                         f"và đổi tên model đúng trong Cài đặt.",
                "action": None,
            })
