...
def build_argv(action: dict) -> list[str]:
    ...
    if atype == "open_path":
        # AI hay trả về path kiểu "~/Documents/x.pdf" — xdg-open KHÔNG tự giãn "~"
        # (đó là việc của shell, mà ở đây cố tình không có shell nào đứng giữa).
        # Không giãn thì xdg-open báo "No such file", và vì open_path chạy qua
        # run_async (không đọc output) nên lỗi này trước đây im lặng hoàn toàn.
        return ["xdg-open", os.path.expanduser(target)]
