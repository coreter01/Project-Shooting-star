...
function applyConfigToUI() {
  ...
  document.getElementById("bgFileName").textContent = config.background_image
    ? config.background_image.split("/").pop()
    : "";
...
async function boot() {
  ...
  document.getElementById("bgUpload").addEventListener("click", async () => {
    // Input HTML thường (<input type="file">) không bao giờ trả về path thật trên
    // đĩa — trình duyệt/webview chặn vì lý do bảo mật, chỉ cho blob: tạm thời không
    // sống sót qua lần khởi động lại. Phải dùng dialog native của Tauri để lấy
    // đường dẫn tuyệt đối thật, mới convertFileSrc() hiển thị đúng được.
    const selected = await openDialog({
      multiple: false,
      filters: [{ name: "Ảnh", extensions: ["png", "jpg", "jpeg", "webp", "gif", "bmp"] }],
    });
    if (!selected) return; // người dùng bấm Cancel
