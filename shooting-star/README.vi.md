...
> **Về tên gọi** (để tránh nhầm lẫn khi đọc code/repo):
> - **Tên project/codebase** (repo, thư mục, package Cargo/npm): `Shooting Star` — không đổi.
> - **Tên chính thức hiển thị cho người dùng** (title bar, menu ứng dụng, .deb): `Astra Assistant`.
> - **Bundle identifier**: `com.astaronova.astra-assistant`.
> - **Tên AI/persona trong chat**: `Astra` (system prompt trong `core/ai_client.py`).
> - **Tên bản snapshot/prototype nội bộ** (trong lúc dev, chưa release): `Trợ Lý Ảo`, đánh số `proto0`, `proto1`, ... — không xuất hiện trong bản release chính thức.
